  ...
  useEffect(() => {
    if (time_range === "last_7_days" && productType) {
      ...
      getBestSale({ type: productType, years: 0, days: null, months: null });
        ...
        getBestSale({ type: productType, months: dayjs().subtract(1, 'month').format('YYYY-MM'), years: null, days: null });
    }else if (time_range === "last_year" && productType){
        getBestSale({ type: productType, years: dayjs().subtract(1, 'year').format('YYYY'), months: null, days: null });
    }
    ...
    if (bestSale) {
      setData({
        datasets: [
          {
            label: "Total",
            data: [bestSale?.total_order_sum, bestSale?.total_sale_sum],
    ...
    <>
      <div className="bg-white rounded-lg shadow-lg h-full">
        <div className="py-5 px-8">
          ...
          <div className="h-full w-full mt-16 flex justify-center items-center">
            <Doughnut data={data} />
          </div>
